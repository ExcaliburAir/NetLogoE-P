...
to go
  ...
  ask turtles [move]
  ask turtles [play]
  ask turtles with [points < 0] [die]
  ask turtles with [points > reproduce-point] [reproduce]
end

to move
  set heading one-of directions
  fd 1
  set points (points - 1)
end
...
to play
  ...
  if (any? nearby) [
    set points points + payoff color [color] of one-of nearby
  ]
end

to reproduce
  ask one-of turtles with-min [points] [die]
  set points 10
  hatch 1
